...
get_and_save_ranked_premia <- function(data, year_to_filter, name_lookup, pop_lookup, output_file_path, output_plot_dir) {
  ...
  # Ensure theta_normalized is numeric for ordering
  year_data_named_pop[, theta_normalized := as.numeric(theta_normalized)]
  
  # Sort by wage premium
  ranked_data_pop <- year_data_named_pop[order(-theta_normalized)] # Descending for top
  
  top_10_df <- head(ranked_data_pop[, .(MSA_Code = met2013, MSA_Name = NAME, Log_Wage_Premium = theta_normalized, Population = total_population)], 10)
  
  bottom_10_from_desc_sort_pop <- tail(ranked_data_pop[, .(MSA_Code = met2013, MSA_Name = NAME, Log_Wage_Premium = theta_normalized, Population = total_population)], 10)
  bottom_10_df <- bottom_10_from_desc_sort_pop[order(Log_Wage_Premium)]
  
  # Print to console
  cat(sprintf("\nTop 10 MSAs by Pooled Log-Wage Premium (theta_normalized) in %d:\n", year_to_filter))
  print(top_10_df)
  cat(sprintf("\nBottom 10 MSAs by Pooled Log-Wage Premium (theta_normalized) in %d:\n", year_to_filter))
  print(bottom_10_df)
  
  # Save to file (text output)
  sink(output_file_path)
  cat(sprintf("Analysis of MSA Pooled Log-Wage Premia for Year: %d\n", year_to_filter))
  cat("=======================================================\n\n")
  cat("Top 10 MSAs by Pooled Log-Wage Premium (theta_normalized) (Population shown in plots):\n")
  print(top_10_df[, .(MSA_Code, MSA_Name, Log_Wage_Premium)]) # Print without pop to text file for brevity
  cat("\nBottom 10 MSAs by Pooled Log-Wage Premium (theta_normalized) (Population shown in plots):\n")
  ...
  plot_top_10 <- ggplot(top_10_df, aes(x = reorder(MSA_Name, -Log_Wage_Premium), y = Log_Wage_Premium)) +
    ...
    coord_flip() +
    labs(title = paste("Top 10 MSAs by Pooled Log-Wage Premium (theta_normalized),", year_to_filter),
         x = "MSA Name", y = "Pooled Log-Wage Premium (theta_normalized)",
  ...
  plot_bottom_10 <- ggplot(bottom_10_df, aes(x = reorder(MSA_Name, Log_Wage_Premium), y = Log_Wage_Premium)) +
    ...
    scale_fill_viridis_c(name = "Population", labels = scales::comma) +
    coord_flip() +
    labs(title = paste("Bottom 10 MSAs by Pooled Log-Wage Premium (theta_normalized),", year_to_filter),
         x = "MSA Name", y = "Pooled Log-Wage Premium (theta_normalized)",
         caption = "Bars colored by population. Population numbers shown.") +
    theme_minimal() +
    theme(axis.text.y = element_text(size = 8))
...
get_ranked_data_for_year <- function(full_wage_data, target_year, name_lkp, pop_lkp, n_msas = 10) {
  ...
  year_data_final[, theta_normalized := as.numeric(theta_normalized)]
  
  ranked_data_all <- year_data_final[order(-theta_normalized)]
  ...
  bottom_n <- bottom_n_from_desc_sort[order(theta_normalized)] # order them by premium for consistency if needed, though not strictly for plotting points
...
if (!is.null(data_2005_ranked) && !is.null(data_2023_ranked)){
  ...
  scatter_plot_combined <- ggplot(combined_ranked_data, 
                                aes(x = total_population, y = theta_normalized, 
                                    color = plot_year, shape = group)) +
    geom_point(size = 3, alpha = 0.8) +
    geom_hline(yintercept = median(combined_ranked_data$theta_normalized, na.rm=TRUE), linetype="dashed", color="grey50") +
    scale_x_log10(labels = scales::comma, name = "Total Population (Log Scale)") +
    scale_y_continuous(name = "Pooled Log-Wage Premium (theta_normalized)") +
    ...
    ggrepel::geom_text_repel(data = subset(combined_ranked_data, Population_missing_flag | total_population > 15e6 | theta_normalized > 0.3 | theta_normalized < -0.3 ), 
